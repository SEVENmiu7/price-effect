...
function createPriceGroup(label,start,end,rawPrices,componentEvidence={electricity:[],service:[]}){return{label,start,end,rawPrices:[...rawPrices],componentEvidence,prices:[],total:null,totalIndex:-1,evidence:"unresolved",needsReview:false,conflict:false,warnings:[]};}
...
function resolvePriceGroupTotals(groups){for(const group of groups){const electricity=group.componentEvidence.electricity;const service=group.componentEvidence.service;if(electricity.length&&service.length){const checkedCount=Math.min(group.rawPrices.length,electricity.length,service.length);const validated=[];for(let index=0;index<checkedCount;index++){if(nearlyEqual(electricity[index]+service[index],group.rawPrices[index]))validated.push(group.rawPrices[index]);}if(validated.length){group.prices=uniqueSorted(validated);group.total=group.prices.length===1?group.prices[0]:null;group.evidence="parallel-formula";if(validated.length<group.rawPrices.length){group.needsReview=true;group.conflict=true;group.warnings.push("部分显示总价与对应电费、服务费之和不一致");}continue;}if(checkedCount===1){const componentSum=electricity[0]+service[0];group.total=group.rawPrices[0];group.totalIndex=0;group.prices=[group.total];group.evidence="explicit-component-conflict";group.needsReview=true;group.conflict=true;group.warnings.push(`总价 ${group.total.toFixed(4)} 与电费、服务费之和 ${componentSum.toFixed(4)} 不一致`);continue;}}
const formula=findFormulaResolution(group.rawPrices);if(!formula)continue;group.total=formula.total;group.totalIndex=formula.index;group.evidence="formula";group.prices=[formula.total];}
for(const group of groups){if(group.prices.length)continue;if(group.rawPrices.length===1){const value=group.rawPrices[0];if(value>=0.25&&value<=3){group.total=value;group.totalIndex=0;group.evidence="single";group.prices=[value];group.needsReview=true;group.warnings.push("价格组只有一个数字，无法校验组成项");}continue;}
const sameLabel=groups.filter(item=>normalizeLabel(item.label)===normalizeLabel(group.label)&&item.rawPrices.length===group.rawPrices.length);const sameShape=groups.filter(item=>item.rawPrices.length===group.rawPrices.length);let templateIndex=dominantTotalIndex(sameLabel,2,.67);if(templateIndex<0)templateIndex=dominantTotalIndex(sameShape,3,.75);if(templateIndex>=0&&templateIndex<group.rawPrices.length){const value=group.rawPrices[templateIndex];if(value>=0.25&&value<=3){group.total=value;group.totalIndex=templateIndex;group.evidence="document-template";group.prices=[value];group.needsReview=true;group.warnings.push("总价根据同一文本的数字结构推断");if(group.rawPrices.length===3){const components=group.rawPrices.filter((_,index)=>index!==templateIndex);if(!nearlyEqual(components[0]+components[1],value)){group.conflict=true;group.warnings.push(`总价 ${value.toFixed(4)} 与组成项之和 ${(components[0]+components[1]).toFixed(4)} 不一致`);}}}}}
for(const group of groups){if(!group.prices.length&&group.rawPrices.length){group.needsReview=true;group.warnings.push("无法从价格组中确认唯一总价");}}
return groups;}
