...
function buildDocumentProfile(timeItems,groups){const emptyProfile={majorityGroupCount:0,majorityLabelSequence:[],majorityRawPriceShape:[],majorityTotalIndexByLabel:{},majorityDirection:"unknown",confidence:0,examples:[]};if(!timeItems.length)return emptyProfile;const groupsByTime=buildProvisionalGroupsByTime(timeItems,groups);const examples=timeItems.map(timeItem=>{const sectionGroups=groupsByTime.get(timeItem.index)||[];return{timeIndex:timeItem.index,timeLabel:timeItem.label,groupCount:sectionGroups.length,labelSequence:sectionGroups.map(group=>group.normalizedLabel||normalizeLabel(group.label)),rawPriceShape:sectionGroups.map(group=>group.rawPriceCount??group.rawPrices.length),direction:profileDirection(sectionGroups,timeItem)};});const groupCountMajority=majorityValue(examples.map(example=>example.groupCount),0);const labelMajority=majorityValue(examples.map(example=>JSON.stringify(example.labelSequence)),"[]");const shapeMajority=majorityValue(examples.map(example=>JSON.stringify(example.rawPriceShape)),"[]");const knownDirections=examples.map(example=>example.direction).filter(direction=>direction!=="unknown");const directionMajority=majorityValue(knownDirections,"unknown");let majorityDirection=directionMajority.value;if(knownDirections.length&&majorityDirection!=="mixed"&&directionMajority.ratio<.67)majorityDirection="mixed";const totalIndexesByLabel=new Map();for(const group of groups){const label=group.normalizedLabel||normalizeLabel(group.label);if(!label||group.totalIndex<0)continue;if(!totalIndexesByLabel.has(label))totalIndexesByLabel.set(label,[]);totalIndexesByLabel.get(label).push(group.totalIndex);}const majorityTotalIndexByLabel={};for(const[label,indexes]of totalIndexesByLabel){majorityTotalIndexByLabel[label]=majorityValue(indexes,-1).value;}const confidenceParts=[groupCountMajority.ratio,labelMajority.ratio,shapeMajority.ratio];if(knownDirections.length)confidenceParts.push(directionMajority.ratio);const confidence=Number((confidenceParts.reduce((sum,value)=>sum+value,0)/confidenceParts.length).toFixed(3));return{majorityGroupCount:groupCountMajority.value,majorityLabelSequence:JSON.parse(labelMajority.value),majorityRawPriceShape:JSON.parse(shapeMajority.value),majorityTotalIndexByLabel,majorityDirection,confidence,examples};}
