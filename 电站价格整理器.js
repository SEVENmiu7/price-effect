...
editor.innerHTML = `<div class="drawer-head"><div><div class="eyebrow">内部工具</div><h2>时段表管理</h2><p>编辑后导出正式配置，再提交 Git 并重新部署。</p></div><button type="button" class="icon-button" data-close-layer aria-label="关闭">×</button></div><div class="editor-access" id="editorAccess"><label>内部密钥<input id="editorKey" type="password" autocomplete="off" placeholder="请输入内部样例密钥"></label><button type="button" class="primary" id="editorUnlock">进入管理</button><p id="editorKeyError" hidden>密钥不正确。</p></div><div id="editorContent" hidden><div class="editor-config-row"><div class="editor-meta"><label>地区标识<input id="editorRegionKey" placeholder="例如 hubei"></label><label>地区名称<input id="editorRegionName" placeholder="例如 湖北"></label><label>生效月份<input id="editorMonth" type="text" inputmode="numeric" maxlength="7" placeholder="2026-06"></label></div><div class="editor-toolbar"><button type="button" id="editorLoadCurrent">载入当前配置</button><button type="button" id="editorNew">空白新建</button><button type="button" id="editorAddPeriod">添加时段</button></div></div><div class="editor-period-head" aria-hidden="true"><span>时段名称</span><span>开始</span><span>结束</span><span>价格级别</span><span>常用</span><span></span></div><div class="editor-periods" id="editorPeriods"></div><div class="editor-feedback" id="editorFeedback">名称可自定义；价格级别仅控制配色。请完整覆盖 00:00—24:00，且时段不能重叠。</div><div class="modal-footer"><button type="button" id="editorValidate">检查时段表</button><button type="button" class="primary" id="editorExport">导出正式配置</button></div></div>`;
