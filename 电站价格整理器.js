...
editor.innerHTML = `<header class="editor-header"><div><div class="editor-kicker">${svgIcon("sliders", "icon icon-sm")}<span>内部工具</span></div><h2>时段表管理</h2><p>编辑后导出正式配置，再提交 Git 并重新部署。</p></div><button type="button" class="icon-button editor-close" data-close-layer aria-label="关闭">${svgIcon("close")}</button></header><div class="editor-access" id="editorAccess"><label>内部密钥<input id="editorKey" type="password" autocomplete="off" placeholder="请输入内部样例密钥"></label><button type="button" class="primary" id="editorUnlock">进入管理</button><p id="editorKeyError" hidden>密钥不正确。</p></div><div id="editorContent" hidden><div class="editor-sections"><section class="editor-section editor-basic-section"><h3 class="editor-section-title">基础信息</h3><div class="editor-config-row"><div class="editor-meta"><label><span>地区标识 ${svgIcon("help", "icon editor-help-icon")}</span><input id="editorRegionKey" placeholder="例如 hubei"></label><label><span>地区名称 ${svgIcon("help", "icon editor-help-icon")}</span><input id="editorRegionName" placeholder="例如 湖北"></label><label><span>生效月份 ${svgIcon("help", "icon editor-help-icon")}</span><span class="editor-month-field">${svgIcon("calendar", "icon icon-sm")}<input id="editorMonth" type="text" inputmode="numeric" maxlength="7" placeholder="2026-06"></span></label></div><span class="editor-config-divider" aria-hidden="true"></span><div class="editor-toolbar"><button type="button" id="editorLoadCurrent">${svgIcon("download", "icon icon-sm")}载入当前配置</button><button type="button" id="editorNew">${svgIcon("file", "icon icon-sm")}空白新建</button><button type="button" class="editor-add-button" id="editorAddPeriod">${svgIcon("plus", "icon icon-sm")}添加时段</button></div></div></section><section class="editor-section editor-period-section"><div class="editor-section-top"><h3 class="editor-section-title">时段列表</h3><button type="button" class="editor-add-button" id="editorAddPeriodInline">${svgIcon("plus", "icon icon-sm")}添加时段</button></div><div class="editor-period-table"><div class="editor-period-head" aria-hidden="true"><span>时段名称 ${svgIcon("help", "icon editor-help-icon")}</span><span>开始 ${svgIcon("help", "icon editor-help-icon")}</span><span></span><span>结束 ${svgIcon("help", "icon editor-help-icon")}</span><span>价格级别 ${svgIcon("help", "icon editor-help-icon")}</span><span>常用</span><span>操作</span></div><div class="editor-periods" id="editorPeriods"></div></div><div class="editor-feedback" id="editorFeedback">${svgIcon("alert-circle", "icon icon-sm")}<span>名称可自定义；价格级别仅控制配色。请完整覆盖 00:00—24:00，且时段不能重叠。</span></div></section></div><div class="modal-footer editor-footer"><button type="button" id="editorValidate">${svgIcon("check", "icon")}检查时段表</button><button type="button" class="primary" id="editorExport">${svgIcon("download", "icon")}导出正式配置</button></div></div>`;
...
editor.querySelector("#editorAddPeriod").addEventListener("click", addEmptyPeriod);
